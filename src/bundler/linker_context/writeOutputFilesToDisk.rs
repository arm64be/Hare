...
pub(crate) fn write_output_files_to_disk(
    ...
    for chunk_index_in_chunks_list in 0..chunks_len {
        ...
        let compiled_output_file: Option<OutputFile> = 'brk: {
            if c.options.generate_bytecode_cache {
                ...
                if loader.is_javascript_like() {
                    ...
                    if c.options.hare {
                        ...
                        if let Err(error) = crate::bundle_v2::dispatch::validate_hare_import(&unit)
                        {
                            let _ = c.log_disjoint().add_error_fmt(
                                None,
                                Loc::EMPTY,
                        ...
                        break 'brk Some(OutputFile::init(OutputFileInit {
                            ...
                            data: OutputFileData::Buffer { data: llvm_ir },
                            side: None,
                            entry_point_index: None,
                            is_executable: false,
                            source_map_index: None,
